...
use App\Models\TeamsProject;
use Illuminate\Http\Request;

class TeamsProjectController extends Controller
{
    public function teams()
    {
        $part = DB::table('teams_projects')
            ->get();
    ...
    public function findTeam($id)
    {
        try{
            $part = DB::table('teams_projects')
                ->join('admins','teams_projects.admin_id','=','admins.id')
                ->select('admins.name', 'admins.phone',  'admins.email',  'admins.user_profile', 'teams_projects.*')
                ->where(['teams_projects.id' => $id])
                ->get();
            if (!$part) {
    ...
    public function updateTeam(Request $request, $id) {
        try {
            $teamdb = TeamsProject::where('id', $id)->first();
            $team = array();
            if($request->hasFile('img')){
                $doc = $request->file('img');
                $fileName = $request->file('img')->getClientOriginalName();
                ...
                $updated = DB::table('teams_projects')->where('id', $request->id)->update($team);
                if ($updated){
                    return response()->json([
                        'status' => true,
    ...
    public function deleteTeam($id) {
        try {
            $part = TeamsProject::where('id', $id)->get();
            if($part) {
                TeamsProject::where('id', $id)->delete();
